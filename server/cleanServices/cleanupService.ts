...
interface FileContainer {
  ...
  media?: Record<string, string> | string | null;
}

// Xoá mọi file trong media (object, array, hoặc JSON string)
async function deleteAllMediaFiles(media: any): Promise<void> {
  if (!media) return;
  let files: string[] = [];
  if (typeof media === 'string') {
    try {
      ...
      // Nếu là chuỗi đơn giản (file path duy nhất)
      files = [media];
    }
  }
  if (typeof media === 'object' && media !== null) {
    files = Object.values(media).filter(f => typeof f === 'string');
  }
...
async function safeDeleteFile(filePath: string): Promise<void> {
  ...
  try {
    ...
    if (!normalizedPath.startsWith(uploadsDir)) {
      console.warn("[Cleanup] Không xoá file ngoài thư mục uploads/public:", filePath);
      return;
    }
    if (fs.existsSync(normalizedPath)) {
      await fs.promises.unlink(normalizedPath);
      console.log("[Cleanup] Đã xoá file:", normalizedPath);
    }
  } catch (error) {
  ...
  try {
    ...
    if (typeof storage.getChatMessagesByUserId === 'function') {
      ...
      for (const msg of messages) {
        await deleteAllMediaFiles(msg.media);
    ...
    if (typeof storage.getCommentsByUserId === 'function') {
      ...
      for (const cmt of comments) {
        await deleteAllMediaFiles(cmt.media);
    ...
    if (typeof storage.getTopicsByUserId === 'function') {
      ...
      for (const topic of topics) {
        await deleteAllMediaFiles(topic.media);
    ...
    if (typeof storage.deleteUser === 'function') {
      ...
      console.log(`[Cleanup] Đã xoá user tạm và dữ liệu liên quan: UserID=${userId}`);
    }
  } catch (error) {
...
export function startCleanupScheduler(): NodeJS.Timeout {
  ...
  setTimeout(autoCleanupTask, 5000); // Run sau 5s khi start
  return setInterval(autoCleanupTask, ONE_DAY_MS);
}